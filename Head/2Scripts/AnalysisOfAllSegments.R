rm(list=ls(all=TRUE))
wd = getwd()
wd = paste(wd, '/Runs-of-Homozygosity-CoverageRoh/Body/3Results/', sep="")
setwd(wd)
...
pdf("../../Body/4Figures/CoverageRoh.PlotterOfAllSegments.R01.pdf", height = 400, width = 600) # Dima - why this -? , onefile = FALSE) ### read wrong the article about this function
par(mfrow=c(22,1))
for (chr in 1:22)
{
  #### chr=1
  WindowsAllForCHR = WindowsAll[WindowsAll$CHR==chr,]
  a = min(WindowsAllForCHR$StartofWindow)
  b = max(WindowsAllForCHR$EndofWindow)
